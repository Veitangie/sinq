...
main() {
    ...
    if [ -w "$INSTALL_DIR" ]; then
        ...
        if [ -t 0 ]; then
            ...
            echo "Non-interactive shell detected. Falling back to $ALT_DIR..."
            mkdir -p "$ALT_DIR"
            mv "$TMP_DIR/$BINARY_NAME" "$ALT_DIR/$BINARY_NAME"
            echo "Note: You may need to add $ALT_DIR to your PATH."
            INSTALL_DIR="$ALT_DIR"
        fi
    fi
